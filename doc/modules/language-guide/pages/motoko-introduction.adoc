...
この考え方は、link:basic-concepts{outfilesuffix}#intro-type-soundness[適切に型付けされた {proglang} プログラムは間違わない] というフレーズでしばしばまとめられます。これは、データに対して実行される操作は、その静的な型によって許可されるものだけであるという意味です。
