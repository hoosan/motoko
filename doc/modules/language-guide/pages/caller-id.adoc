...
{proglang} の Shared 関数は、シンプルな Caller（関数の呼び出し元）の証明をサポートしており、これにより関数の Caller に関連付けられた {IC} の **プリンシパル** を検査することが可能になります。
...
プリンシパルに関するその他の操作は、link:../base-libraries/principal{outfilesuffix}[Principal] ベースライブラリを参照してください。
