...
{proglang} コンパイラ、ドキュメント、また他のツールは、 https://github.com/dfinity/motoko[オープンソース] であり、Apache 2.0 ライセンスを得てリリースされています。コントリビュートは歓迎です。
...
{proglang} は、{IC} Canister スマートコントラクトへの、ネイテイブサポートを持っています。

Canister スマートコントラクト (または略して Canister) は、 {proglang} Actor として表されます。 Actor とは、そのステータスを完全にカプセル化して、非同期メッセージでのみ別の Actor との通信を行う自律的なオブジェクトです。
...
{proglang} コンパイラー（かつ SDK ）は、Candid と呼ばれる言語に依存しないフォーマットでこのインターフェースを出力するので、Candid をサポートしている別の Canisters やブラウザ上のコードやスマートフォンアプリは、Actor のサービスを利用することができる。{proglang} コンパイラは、Candid ファイルを消費かつ生産でき、{proglang} にシームレスに他の言語（ Candid サポートから提供された）で埋め込まれた Canister と接続することを許可します。

例えば、以前の {proglang} の `+Counter+` Actor は、次に続く Candid インターフェースを持っています。
...
{IC} は、作動している Canister のメモリと他のステータスも保持しています。それゆえ、Motoko Actor のステータスは、そのインメモリデータ構造も含め永久に残り続けます。Actor のステータスは、それぞれのメッセージと共に復元することや外部ストレージに保存することを明確に必要としていません。

例えば、シーケンシャルなIDをテキストの名前に割り当てる次の `Registry` Actor (Canister) では、Actor のステータスがたくさんの {IC} ノードマシーンで複製されたもので、一般的にメモリ内にはいないれども、ハッシュテーブルのステータスはコールを介して保存されています。
...
それは、例えばより高価な導入のような中断はなく、インストールされ、_n_ 回インクリメントされ、そうしてアップグレードされます。
...
`+value+` は `stable` として宣言されていたので、現在のステータスやサービスの n はアップグレードの後でも保持されています。カウンティングは、0から再度始まるのではなく、n 回目から始まります。
...
stable な変数の使用のみでは解決できないシナリオために、{proglang} は、アップグレードの前後で即座に動作するかつ任意のステータスを静的な変数にすることを許可する、ユーザーが定義できるアップグレードフックを提供しています。


[.card]
== さらなる機能

{proglang} は、派生形、任意精度演算、またはガベージコレクションを含めた、多くの開発者の生産性を上げる機能を提供しています。

{proglang} は、スマートコントラクト Canister を導入するためだけの言語ではなく、またそうであることを意図していません。もしあなたのニーズを満たさない時のために、Rust プログラミング言語の CDK があります。
私達の目標は、 {IC} 上やニュートラル Candid インターフェースを通した他国の Canister スマートコントラクトと一緒に動作する Canister スマートコントラクトをいかなる言語でも作成できるようにすることです。
