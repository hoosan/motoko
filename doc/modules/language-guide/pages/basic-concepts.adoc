...
この章では、以下のトピックについて説明します：
 ...
 - <<intro-printing,数字のテキストの表示>> と <<intro-stdlib, ベースライブラリの使い方>>
...
各 {proglang} _プログラム_ は、宣言と式を自由に組み合わせたもので、それらの統語的な分類は異なりますが、互いに関連しています（プログラムの正確な構文については、link:language-manual{outfilesuffix}[言語のクイックリファレンスガイド]を参照して下さい）。
...
各 {proglang} _プログラム_ は、宣言と式を自由に組み合わせたもので、それらの統語的な分類は異なるものの互いに関連しているということを<<intro-progs,思い出しましょう>>。 
...
`1 + 1` の型は `Nat` であって `Text` ではなく、また `Nat` と `Text` は派生型の関係にないので、型アノテーションの `Text` はその後のプログラムと一致しません。
...
[[intro-stdlib]]
=== {proglang} ベースライブラリの使い方

言語のエンジニアリングにおけるさまざまな実用上の理由から、{proglang} の設計では組み込みの型や操作を最小限に抑えるようにしています。

その代わり、{proglang} ベースライブラリは、言語を完全なものにするための型や操作を可能な限り提供しています。
__**ただし**、このベースライブラリは開発中であり、まだ不完全です。__

link:base-libraries/stdlib-intro{outfilesuffix}[Motoko ベースライブラリ]では、{proglang} ベースライブラリからのモジュールを _選定して_ 示していますが、これは例題で使われているコアな機能に焦点を当てたもので、抜本的な変更はないと考えられます。
しかし、これらのベースライブラリの API はすべて時間の経過とともに（程度の差こそあれ）確実に変化し、特にサイズと数が大きくなっていくでしょう。

ベースライブラリからインポートするには、`import` キーワードを使います。
...
このような状況に対応するために、ベースライブラリである `Prelude` の `xxx`, `nyi`, `unreachable` 関数を、後述のように使用することができます。
...
コードパスを、論理的に不可能あるいは _到達不可能_ なものとして記録するには、ベースライブラリの関数である `unreachable` を使います：
...
ゼロ除算、配列の境界外へのインデックス、パターンマッチの失敗などのエラーは型システムでは防ぐことができませんが、実行時に _トラップ_ と呼ばれるエラーを引き起こす可能性があります。
